...
function renderAiAnalysis(paper) {
  ...
  wrapper.setAttribute("aria-label", "AI 生成的中文摘要分析");

  const title = document.createElement("h3");
  title.textContent = "AI 生成的中文摘要分析";

  const list = document.createElement("dl");
  [
    ["科学问题", analysis.scientific_question],
    ["关键发现", analysis.key_highlight],
    ["主要局限", analysis.main_limitation],
    ["研究启示", analysis.research_implication],
  ].forEach(([label, value]) => {
    if (!value) return;
