...
function renderAiAnalysis(paper) {
  ...
  const steps = analysis.methodology_steps || analysis.methodologySteps;
  if (steps) {
    const stepArray = steps.split(";").map((s) => s.trim()).filter(Boolean);
    if (stepArray.length > 0) {
      const flow = document.createElement("div");
      flow.className = "methodology-flow";
      stepArray.forEach((step, i) => {
        ...
        stepEl.className = "methodology-step";
...
function getAiAnalysis(paper) {
  ...
  const values = {
    ...
    methodology_steps: analysis.methodology_steps || analysis.methodologySteps,
