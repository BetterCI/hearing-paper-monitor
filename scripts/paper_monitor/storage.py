...
CREATE TABLE IF NOT EXISTS papers (
    ...
    pubmed_full_text_image_checked_at TEXT,
...
MIGRATIONS = [
    ...
    "ALTER TABLE papers ADD COLUMN open_access INTEGER",
    ...
    "ALTER TABLE papers ADD COLUMN pubmed_full_text_image_checked_at TEXT",
    "ALTER TABLE papers ADD COLUMN media_checked_at TEXT",
    ...
    for item in payload.get("papers", []):
        ...
        conn.execute(
            ...
            (
                ...
                _bool_or_none(item.get("open_access")),
                ...
                item.get("pubmed_full_text_image_checked_at"),
                item.get("media_checked_at"),
    ...
    for paper in papers:
        ...
        conn.execute(
            ...
            ON CONFLICT(id) DO UPDATE SET
                ...
                pubmed_full_text_url=COALESCE(excluded.pubmed_full_text_url, papers.pubmed_full_text_url),
                pubmed_full_text_source=COALESCE(excluded.pubmed_full_text_source, papers.pubmed_full_text_source),
                pubmed_full_text_image_checked_at=COALESCE(papers.pubmed_full_text_image_checked_at, excluded.pubmed_full_text_image_checked_at),
                media_checked_at=COALESCE(papers.media_checked_at, excluded.media_checked_at),
            ...
            (
                ...
                _bool_or_none(paper.open_access),
                ...
                paper.pubmed_full_text_image_checked_at,
                paper.media_checked_at,
...
def all_papers(conn: sqlite3.Connection) -> list[dict]:
    ...
    columns = [
        ...
        "pubmed_full_text_image_checked_at",
        "media_checked_at",
        "publication_stage",
        "key_image_url",
        "key_image_alt",
    ...
    for row in rows:
        ...
        if item.get("open_access") is not None:
            item["open_access"] = bool(item["open_access"])
        if item.get("pubmed_full_text_available") is not None:
            item["pubmed_full_text_available"] = bool(item["pubmed_full_text_available"])
        ...
        if item.get("open_access") is not True:
            item.pop("open_access", None)
        if not item.get("open_access_url"):
            item.pop("open_access_url", None)
        if not item.get("open_access_source"):
        ...
        if not item.get("pubmed_full_text_url"):
            item.pop("pubmed_full_text_url", None)
        if not item.get("pubmed_full_text_source"):
            item.pop("pubmed_full_text_source", None)
        if not item.get("pubmed_full_text_image_checked_at"):
            item.pop("pubmed_full_text_image_checked_at", None)
        if not item.get("media_checked_at"):
